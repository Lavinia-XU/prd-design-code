...
- 缺少业务设计Skill、业务设计Skill未覆盖导航结构或菜单归属不清时，是否按需读取产品导航索引；产品导航索引是否仅作为导航归属和菜单命名兜底参考，没有覆盖用户输入、业务设计Skill或已有代码菜单配置。
- 导航结构是否综合展示本次Demo覆盖范围，不按每个页面、弹窗或抽屉重复输出。
- 页面总览表是否列清所有页面或容器，并标明设计来源和编码方式；设计来源用于说明来自业务设计Skill还是通用设计Skill，编码方式应与页面级AI Coding指导保持一致，取值为“复用框架”“直接引用”“组件复用”或“全新开发”。
...
- Wireframe / ASCII线框图是否与页面内容区块描述一致。
...
- 当需求只描述局部操作时，是否判断该操作依赖的上下文能力，并从业务设计Skill、主题设计、功能点设计、基础设计、已有代码或AI业务理解中按需补齐。
...
22. HTML线框图未先读取 [页面类型与页面元素说明](../04-design-basics/01-page-types.md) 中对应页面类型的页面结构，只画空白卡片、通用容器或与页面内容区块不一致的布局。
...
- Coding输入是否综合HTML设计说明书、原始需求资料、待确认问题回复、业务设计Skill、必要时的通用设计库、功能点设计和已有Demo代码，禁止把HTML当作唯一输入或机械执行稿。
