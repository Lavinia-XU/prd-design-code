...
- 带固定选项的下拉/选择类字段（表单下拉、筛选下拉、单选、多选等枚举控件）必须把需求文档中的选项**逐项完整列出**，禁止用“等/例如/如：/…”只举例；页面对象写入 requirementOptionSets（字段名到完整选项列表的声明），声明选项未全部落入表单 `rules`（选项/规则）或筛选 `options`（选项范围）单元格会被校验器以 RULE-48 阻断，未声明选项集的选择类字段出现截断标记会被 RULE-48 告警（详见同一章节 11.13）。
...
- 生成HTML前必须运行设计闭环自动校验（`python scripts/validate_demo_spec.py --input demo-spec.json --template-registry references/02-template-contracts/common-design-template-registry.json --strict`）：页面清单闭环（RULE-28）、操作目标闭环（RULE-29）、Tab 变体闭环（RULE-30，仅多内容 Tab 页面强制）、页面级 Coding 闭环（RULE-31）、表格与详情字段一致性闭环（RULE-38，表格有详情容器时表格展示字段必须能在详情中找到）、表格标签使用约束闭环（RULE-39，同一表格内标签不超过 5 个，深色/icon/点状标签各仅允许 1 次、浅色标签最多 2 次，中性描述字段不占用标签配额）、设计依据一致性闭环（RULE-43，未读/仅索引/引用未精读章节、任意能力覆盖未落地）、未核验实现细节隔离闭环（RULE-44，partial/unavailable 下不得出现未核验的真实导出名、产品专有组件名或真实路径）、线框底部按钮闭环（RULE-37，底部操作区须按模板 buttonOrder 绘制模板按钮，仅画关闭、漏画主操作、顺序或文案不一致、未声明 override 的业务自定义按钮均阻断）、线框结构一致性（RULE-46/RULE-47，同一区域重复绘制或内容行右边界错位提示）、下拉选项完整性闭环（RULE-48，固定选项型下拉/选择类字段须逐项完整列出选项，声明 requirementOptionSets 时缺项阻断，未声明选项集出现 等/例如/… 截断指纹告警）、详情页字段去重闭环（RULE-49，详情类页面顶部概览卡片/对象摘要已展示字段不得在其下方详情描述列表重复展示，声明 detailSummaryFields 为准、缺省以 cardFields 兜底，命中告警 DETAIL_FIELD_DUPLICATE，同一描述列表内字段重复阻断 DETAIL_FIELD_DUPLICATED_IN_LIST）；error 级问题禁止生成HTML，warning 级不阻断，info 级仅提示待核验。
...
- 下拉选项完整性（RULE-48）：固定选项型下拉/选择类字段（表单下拉、筛选下拉、单选、多选等枚举控件）已在“选项/规则”（表单 `rules`）或“选项范围”（筛选 `options`）单元格逐项完整列出需求文档中的选项，未用“等/例如/如：/…”只举例；声明了 `requirementOptionSets` 的页面，其声明的完整选项已全部落位（缺项被 RULE-48 以 REQUIRED_OPTION_MISSING 阻断，声明字段不存在以 OPTION_FIELD_NOT_FOUND 阻断），未声明选项集的选择类字段未出现截断指纹（出现以 OPTION_TRUNCATION_MARKER 告警）。
- 详情页字段去重（RULE-49）：详情类页面（详情抽屉页/下钻详情页/详情弹窗）顶部概览卡片/对象摘要已展示的字段（声明 `detailSummaryFields` 为准，未声明时以 `cardFields` 兜底）未在其下方详情描述列表（概览/描述/detail 等区块）重复展示（命中以 DETAIL_FIELD_DUPLICATE 告警，确需有意重复时用 `detailDedupExempt` 声明豁免）；同一描述列表区块内字段名未重复（重复以 DETAIL_FIELD_DUPLICATED_IN_LIST 阻断）；历史/日志/操作记录类区块与表格字段不参与去重，非详情页不校验。
...
- Template validator：见 [scripts/validate_demo_spec.py](scripts/validate_demo_spec.py)，校验模板契约与线框结构（49 项规则，含设计依据一致性 RULE-43、未核验实现细节隔离 RULE-44、线框与区域结构双向一致性 RULE-45/46、下拉选项完整性 RULE-48、详情页字段去重 RULE-49），输出结构化 JSON 错误与修复建议；参数为`--input`、`--template-registry`和`--strict`，校验失败返回非 0 exit code。
