...
7. 页面拆解时必须先判断功能是独立业务旅程、菜单级能力、Tab级能力，还是依附于已有页面的轻量入口；非核心旅程中的辅助配置、全局配置或对象关联配置，不要因为出现独立能力名称就默认新增菜单或独立页面，优先评估放在相关页面内作为按钮或工具入口，并通过弹窗、抽屉或必要的下钻页面承载。
8. 生成页面关键交互说明时，必须综合五类信息：需求资料中明确写出的操作、流程、状态和限制；业务设计Skill中的既有产品规范；可用Demo代码环境中的已有模块实现；通用设计库中的B端页面设计模式；AI基于业务目标、用户任务和页面容器类型做出的合理补齐。
9. 对话框主体只输出到“页面总览表”为止，禁止在对话框继续输出逐页设计说明、Demo交互与逻辑规则、Mock数据细节或AI Coding完整提示词。
